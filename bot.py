...
# Admin có thể thêm trực tiếp bằng lệnh /kho hoặc thêm sẵn ở đây
product_stock = {
...
# Các gói chi tiết riêng cho danh mục MENU
menu_sub_items = {
    "menu_ffhax": [
...
def manage_stock(message):
    if message.from_user.id != ADMIN_ID:
        return
    try:
        # Cú pháp: /kho id_san_pham | noi_dung_key_hoac_link
        ...
        bot.reply_to(message, f"✅ Đã thêm thành công vào kho của ID: `{item_id}`\n📦 Nội dung: `{item_value}`\n📊 Tổng kho hiện tại: `{len(product_stock[item_id])}` sản phẩm.", parse_mode="Markdown")
    except Exception as e:
...
def process_purchase(call, item):
    ...
    # Kiểm tra kho hàng xem còn hàng không
    ...
    # Lấy hàng tự động từ kho và xóa đi 1 phần tử
